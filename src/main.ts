...
class Soundcraft extends utils.Adapter {
    ...
    private async createObjectStructure(): Promise<void> {
        ...
        // Create muteGroup folder/device
    ...
    private subscribeMixerStates(): void {
        if (!this.mixer) {
            return;
        }
        ...
        this.subscriptions.push(
            this.mixer.master.faderLevel$.subscribe(
                (val: number) => void this.setStateAsync('master.faderLevel', { val, ack: true }),
            ),
            this.mixer.master.pan$.subscribe(
                (val: number) => void this.setStateAsync('master.pan', { val, ack: true }),
            ),
            this.mixer.master.dim$.subscribe(
                (val: number) => void this.setStateAsync('master.dim', { val, ack: true }),
            ),
        );
        ...
        for (let i = 0; i < this.mixerChannels.hw; i++) {
            ...
            this.subscriptions.push(
                ch.faderLevel$.subscribe(
                    (val: number) => void this.setStateAsync(`${prefix}.faderLevel`, { val, ack: true }),
                ),
                ch.mute$.subscribe(
                    (val: number) => void this.setStateAsync(`${prefix}.mute`, { val: Boolean(val), ack: true }),
                ),
                ch.pan$.subscribe((val: number) => void this.setStateAsync(`${prefix}.pan`, { val, ack: true })),
                hwCh.gain$.subscribe((val: number) => void this.setStateAsync(`${prefix}.gain`, { val, ack: true })),
                hwCh.phantom$.subscribe(
                    (val: number) => void this.setStateAsync(`${prefix}.phantom`, { val: Boolean(val), ack: true }),
                ),
            );
        }
        ...
        for (let i = 0; i < this.mixerChannels.aux; i++) {
            ...
            this.subscriptions.push(
                ...
                auxMaster.pan$.subscribe((val: number) => void this.setStateAsync(`${prefix}.pan`, { val, ack: true })),
            );
            ...
            for (let j = 0; j < this.mixerChannels.hw; j++) {
                ...
                this.subscriptions.push(
                    auxInput.faderLevel$.subscribe(
                        (val: number) => void this.setStateAsync(`${inputPrefix}.faderLevel`, { val, ack: true }),
                    ),
                    auxInput.mute$.subscribe(
                        (val: number) =>
                            void this.setStateAsync(`${inputPrefix}.mute`, { val: Boolean(val), ack: true }),
                    ),
                    auxInput.pan$.subscribe(
                        (val: number) => void this.setStateAsync(`${inputPrefix}.pan`, { val, ack: true }),
                    ),
                );
            }
        }
        ...
        for (let i = 1; i <= this.mixerChannels.muteGroups; i++) {
            ...
            this.subscriptions.push(
                muteGroup.state$.subscribe(
                    (val: number) => void this.setStateAsync(`muteGroup.${i}`, { val: Boolean(val), ack: true }),
                ),
            );
        }
